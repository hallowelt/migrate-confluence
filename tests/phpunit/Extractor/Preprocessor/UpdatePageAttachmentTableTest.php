...
class UpdatePageAttachmentTableTest extends TestCase {
	...
	public function testCreatesPageAttachmentEntry(): void {
		...
		$workspaceDB->addPage( 600, 10, 'Page', 'TEST:Page', 'current', '', '', '1', -1, -1, [], [], [], [] );
		$workspaceDB->addAttachment( 601, 10, 'file.txt', 'txt', 600, 'current', '1', '', '', -1, '/tmp/a', [], [], [] );
		...
		$this->assertSame( 601, $pageAttachments[0]['attachment_id'], 'Unexpected attachment_id for page attachment.' );
		$this->assertSame( 600, $pageAttachments[0]['page_id'], 'Unexpected page_id for page attachment.' );
		$this->assertSame( 'file.txt', $pageAttachments[0]['original_attachment_filename'], 'Unexpected original filename for page attachment.' );
