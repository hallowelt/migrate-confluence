...
class PopulateAdditionalAttachmentsTableTest extends TestCase {
	...
	public function testAddsOnlyUnknownCurrentAttachmentsToAdditionalTable(): void {
		...
		$workspaceDB->addAttachment( 801, 10, 'known.pdf', 'pdf', 800, 'current', '1', '', '', -1, '/tmp/c', [], [], [] );
		$workspaceDB->addAttachment( 802, 10, 'extra.pdf', 'pdf', 800, 'current', '1', '', '', -1, '/tmp/d', [], [], [] );
		...
		$this->assertSame( 802, $additionalAttachments[0]['attachment_id'], 'Expected only unknown attachment to be added.' );
		$this->assertSame( 'extra.pdf', $additionalAttachments[0]['original_attachment_filename'], 'Unexpected original filename for additional attachment.' );
		$this->assertNotSame( '', $additionalAttachments[0]['target_attachment_filename'], 'Expected target filename for additional attachment.' );
	}
}
