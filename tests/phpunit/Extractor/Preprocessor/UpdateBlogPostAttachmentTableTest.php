...
class UpdateBlogPostAttachmentTableTest extends TestCase {
	...
	public function testCreatesBlogPostAttachmentEntry(): void {
		...
		$workspaceDB->addAttachment( 701, 42, 'image.png', 'png', 700, 'current', '1', '', '', -1, '/tmp/b', [], [], [] );
		...
		$this->assertSame( 701, $blogPostAttachments[0]['attachment_id'], 'Unexpected attachment_id for blog post attachment.' );
		$this->assertSame( 700, $blogPostAttachments[0]['blog_post_id'], 'Unexpected blog_post_id for blog post attachment.' );
		$this->assertSame( 'image.png', $blogPostAttachments[0]['original_attachment_filename'], 'Unexpected original filename for blog post attachment.' );
