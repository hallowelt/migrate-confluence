...
class WikisOptionHelper {
	...
	public function validateFile(): ?string {
		...
		$parser = new WikisOptionCSVParser();
		$wikiConfig = $parser->parseWikiConfigCSV( $filename );
		foreach ( $wikiConfig as $config ) {
			if ( !isset( $config['space-key'] ) || empty( $config['space-key'] ) ) {
				return "Config file '$filename' is missing 'space-key' for a space.";
			}
