...
class PlaceholderManager {
	...
	private array $flippedPlaceholders = [];
	...
	public function getPlaceholder( string $string ): string {
		...
		$this->placeholders[$string] = $key;
		$this->flippedPlaceholders[$key] = $string;
	 ...
	 * @return string|null
	 */
	public function replacePlaceholders( string $wikiText ): ?string {
		foreach ( $this->flippedPlaceholders as $placeholder => $original ) {
			$wikiText = str_replace( $placeholder, $original, $wikiText );
		}
		return $wikiText;
	}
}
