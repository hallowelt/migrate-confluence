 ...
 */
class PageComments extends ContentProcessorBase {
	...
	private function addCommentPages(): void {
		...
		$pageIdToCommentsMap = $this->getContentIdToCommentsMap( $comments );
		...
		foreach ( $pageIdToCommentsMap as $pageId => $comments ) {
			...
			if ( $this->skip( $wikiTitle, $talkTitle ) ) {
	...
	protected function skip( string $wikiTitle, string $talkTitle ): bool {
		if ( $this->skipHelper->skipPage( $wikiTitle )
	...
	/**
	 * Talk pages are virtual targets during compose and should only be skipped by configuration,
	 * not by DB invalidity checks against source content tables.
	 *
	 * @param string $talkTitle
	 * @return bool
	 */
	protected function skipTalkTitleByConfiguration( string $talkTitle ): bool {
		$namespace = 'NS_MAIN';
		if ( str_contains( $talkTitle, ':' ) ) {
			$namespace = substr( $talkTitle, 0, strpos( $talkTitle, ':' ) );
		}
		...
		if ( in_array( $talkTitle, $this->migrationConfig->getComposerSkipTitles() ) ) {
			return true;
		}
	 ...
	 * @return array
	 */
	private function getContentIdToCommentsMap( array $comments ): array {
		$pageIdToCommentsMap = [];
		foreach ( $comments as $comment ) {
			...
			if ( $this->skipHelper->skipWikiTitle( $wikiTitle ) ) {
				$this->output->writeln( "Skip comments for title $wikiTitle." );
				continue;
			}
