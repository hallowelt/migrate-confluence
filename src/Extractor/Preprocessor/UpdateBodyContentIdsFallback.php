...
class UpdateBodyContentIdsFallback extends ProcessorBase {
	...
	private function updatePagesTable(): void {
		...
		foreach ( $pages as $page ) {
			...
			$foundIds = $this->reader->getBodyContentIdsForContentId( $pageId );
			if ( empty( $foundIds ) ) {
				continue;
			}
	...
	private function updateCommentsTable(): void {
		...
		foreach ( $comments as $comment ) {
			...
			$foundIds = $this->reader->getBodyContentIdsForContentId( $commentId );
	...
	private function updateSpaceDescriptionsTable(): void {
		$spaceDescriptions = $this->reader->getSpaceDescriptions();
		foreach ( $spaceDescriptions as $spaceDesc ) {
			...
			$foundIds = $this->reader->getBodyContentIdsForContentId( $spaceDescriptionId );
			if ( empty( $foundIds ) ) {
				continue;
			}
